...
CREATE TABLE public.dm_threads (
  ...
  CONSTRAINT dm_pair_ordered CHECK (user_a IS NULL OR user_b IS NULL OR user_a < user_b)
);
...
CREATE POLICY "users can log their own failures" ON public.tool_failure_log FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
CREATE POLICY "users can view their own failures" ON public.tool_failure_log FOR SELECT TO authenticated USING (auth.uid() = user_id OR public.has_role(auth.uid(), 'admin'));
...
CREATE POLICY "users can submit ocr corrections" ON public.ocr_corrections FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
CREATE POLICY "users can view their own ocr corrections" ON public.ocr_corrections FOR SELECT TO authenticated USING (auth.uid() = user_id OR public.has_role(auth.uid(), 'admin'));
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    UPDATE public.post_comments SET like_count = like_count + 1 WHERE id = NEW.comment_id; RETURN NEW;
  ELSIF TG_OP = 'DELETE' THEN
    UPDATE public.post_comments SET like_count = GREATEST(like_count - 1, 0) WHERE id = OLD.comment_id; RETURN OLD;
  END IF; RETURN NULL;
END $$;
CREATE TRIGGER trg_comment_like_count AFTER INSERT OR DELETE ON public.post_comment_likes FOR EACH ROW EXECUTE FUNCTION public.adjust_comment_like_count();
...
CREATE TABLE public.library_books (
  ...
  author text,
  cover_url text,
  category text NOT NULL CHECK (category = ANY (ARRAY['novel'::text, 'book'::text, 'comics'::text, 'poetry'::text, 'course'::text])),
  read_url text,
  description text,
...
CREATE POLICY "user reads own book purchases" ON public.library_book_purchases FOR SELECT USING (auth.uid() = user_id);

CREATE OR REPLACE FUNCTION public.purchase_library_book(_book_id uuid)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE _uid uuid := auth.uid(); _book public.library_books; _new_balance int;
BEGIN
  IF _uid IS NULL THEN RAISE EXCEPTION 'Not authenticated'; END IF;
...
GRANT SELECT ON public.library_courses TO anon, authenticated;
...
INSERT INTO storage.buckets (id, name, public) VALUES ('book-pdfs', 'book-pdfs', true) ON CONFLICT (id) DO NOTHING;
...
CREATE POLICY "users manage own thread reads" ON public.dm_thread_reads FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
