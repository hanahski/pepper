...
ALTER TABLE public.study_notes ADD CONSTRAINT study_notes_uploader_id_fkey FOREIGN KEY (uploader_id) REFERENCES public.profiles(id) ON DELETE SET NULL;
...
ALTER TABLE public.profiles ADD COLUMN seen_welcome boolean NOT NULL DEFAULT false;
...
DO $$ BEGIN ALTER PUBLICATION supabase_realtime ADD TABLE public.dm_thread_members; EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
CREATE POLICY "post-media auth upload" ON storage.objects FOR INSERT TO authenticated WITH CHECK (bucket_id = 'post-media' AND auth.uid()::text = (storage.foldername(name))[1]);
CREATE POLICY "post-media owner delete" ON storage.objects FOR DELETE TO authenticated USING (bucket_id = 'post-media' AND auth.uid()::text = (storage.foldername(name))[1]);